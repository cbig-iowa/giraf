function [x,stats] = opt_giraf_flex(x0,b,A,At,sampmask,param,settings)
...
R = settings.R; %neighborhood radius size;
res = settings.res;


%define custom filter shape
h = get_circular_filter(R);
filter_siz = size(h);
...
for i=1:iter
    ...
    gradx = M(x_pad);
    G = build_gram_matrix_flex(gradx,h,overres);
    tic;
    ...
    mu = build_sos_poly_flex(U,s,q,overres,h);

    %step 2: ADMM solution of least squares problem
    gam = max(mu(:))/gam_fac;  %set ADMM parameter to one-tenth of sos-mask max value
    [x_pad,admm_res] = run_ADMM_WL2(x_pad,mu,M,Mt,sampmask_pad,MtMmask,Atb_pad,gam,lambda,ADMM_iter,ADMM_tol);
...
% stats.MSE = [(norm(Atb-x0)/norm(x0))^2, stats.MSE];
